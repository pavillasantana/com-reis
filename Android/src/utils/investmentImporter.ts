 ...
 * Espelho do parser web (web/src/utils/investmentImporter.ts).
 * Tolerante a formatos de corretoras/B3, inclusive "PETR4 - PETROBRAS".
 */
...
export function parseAportesXLSXBase64(base64: string): PendingAporte[] {
...
export function parseAtivosXLSXBase64(base64: string): PendingAtivo[] {
  const wb = XLSX.read(base64, { type: 'base64', cellDates: false });
  const sheet = wb.Sheets[wb.SheetNames[0]];
  if (!sheet) return [];
  const rows = XLSX.utils.sheet_to_json<Record<string, unknown>>(sheet, { defval: '' });
...
/**
 * Ponto de entrada unificado: decide CSV vs XLSX pela extensão do arquivo.
 * `text` = conteúdo lido do arquivo (texto UTF-8 para CSV/TSV, base64 para XLSX).
 */
export function parseInvestmentContent(
  text: string,
  fileName: string,
  mode: 'ativos' | 'aportes'
  ...
  return mode === 'aportes' ? parseAportesXLSXBase64(text) : parseAtivosXLSXBase64(text);
}
